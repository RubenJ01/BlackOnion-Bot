...
public class Config {
	...
	// DON'T make this final, or else it'll break
	private final BotMetadata metadata = ConfigManager.metadata;
	...
	public static <T> T get(String name, Class<T> clazz, IFlag... flagsArr) {
		...
		for (IFlag f : flags) {
			...
			} else if (f instanceof Flags.Range flag && result instanceof Number num && (num.doubleValue() < flag.min() || num.doubleValue() > flag.max())) {
				throw new IllegalArgumentException("Config value " + name + " is out of range " + flag.min() + " - " + flag.max());
			}
		}
		return result;
	}
