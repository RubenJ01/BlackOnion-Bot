...
public class SupportCommand extends SlashCommand {
	...
	private static final String bot_developers = Config.getInstance().getMetadata().authors().entrySet().stream().map(entry -> "[" + entry.getKey() + "](" + entry.getValue() + ")").collect(Collectors.joining(", "));
	private static final String blackonion_authors = Config.getInstance().getMetadata().blackonion_authors().entrySet().stream().map(entry -> "[" + entry.getKey() + "](" + entry.getValue() + ")").collect(Collectors.joining(", "));
